...
const addTreasuryOverview = (doc, daoData, pageWidth, startY) => {
  ...
  addFormattedText(doc, `$${daoData.balanceUSD} (${daoData.balance} ETH)`, col1X, yPos + 10, {
  ...
  addFormattedText(doc, `${daoData.transactions30d} transactions`, col2X, yPos + 10, {
    fontSize: PDF_STYLES.fonts.medium,
    fontStyle: 'bold',
    color: PDF_STYLES.colors.dark
  });
  ...
  addFormattedText(doc, `Since ${daoData.walletAge}`, col2X, yPos + 10, {
    fontSize: PDF_STYLES.fonts.medium,
    fontStyle: 'bold',
    color: PDF_STYLES.colors.dark
  });
 ...
 */
const addPDFFooter = (doc, pageWidth, pageHeight) => {
  ...
  addFormattedText(doc, 'This report is generated by scor - Professional DAO Risk Assessment Platform', 15, footerY + 10);
  addFormattedText(doc, 'Analysis based on live blockchain data from Etherscan and CoinGecko APIs', 15, footerY + 18);
  addFormattedText(doc, `For questions about this analysis, visit: ${window.location.origin}`, 15, footerY + 26);
  
  // Right side footer
  addFormattedText(doc, 'CONFIDENTIAL', pageWidth - 25, footerY + 10, { fontStyle: 'bold' });
  addFormattedText(doc, 'Page 1 of 1', pageWidth - 25, footerY + 18);
};
...
export const generateProfessionalPDFReport = (daoData, options = {}) => {
  ...
  try {
    ...
    // Generate filename
    const currentDate = new Date().toLocaleDateString();
    const safeDAOName = daoData.name.replace(/[^a-zA-Z0-9]/g, '_');
    const fileName = `${safeDAOName}_Risk_Assessment_${currentDate.replace(/\//g, '-')}.pdf`;
...
export const generateQuickSummaryPDF = (daoData) => {
  ...
  // Implementation would be similar but condensed
